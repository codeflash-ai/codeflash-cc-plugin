...
The `--worktree` flag is **always** passed. It tells codeflash to work in a separate git worktree to avoid interfering with the user's working directory. the --no-pr flag is always passed.
